...
for i in range(tests):
	...
	for line in f:
		if(len(line)>1):
			...
			tmp = line.split(',')
			elements.append(tmp)
			if(tmp[4] not in classes):
				classes.append(tmp[4])
	...
	for i in range(len(test)):
		...
		for tru in train:
			...
			if(tmp2<d):
				...
				ans = tru[4]
		res.append(ans)
		confusion[test[i][4]][ans] += 1 
		if(ans==test[i][4]):
			hit += 1
			fans.write('HIT -->  calculated - '+ans+'  expected - '+test[i][4]+'\n')
		else:
			miss +=1
			fans.write('MISS -->  calculated - '+ans+'  expected - '+test[i][4]+'\n')	
