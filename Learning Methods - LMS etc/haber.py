...
def crossValidate(complete,folds,algo,comb):
	...
	for i in range(folds):
		...
		for si in confusion[1]:
			for sj in confusion[1][si]:
				finalConf[si][sj] = finalConf[si][sj] + confusion[1][si][sj]
	avErr = errSum/folds
	print "error",avErr
	sqrSum = sqrSum/folds
	...
	for ki in finalConf:
	    for kj in finalConf[ki]:
	    	finalConf[ki][kj] = finalConf[ki][kj]/folds
	print "Confusion Matrix",finalConf


a = Dataset()
a.readData("haberman.data")
a.writeData("haber")
#l = splitDataset(a,4)
#l[0].writeData("1")
#l[1].writeData("2")
